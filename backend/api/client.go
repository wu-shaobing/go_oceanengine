...
func (c *APIClient) call(ctx context.Context, request *http.Request, response interface{}) (*http.Response, error) {
	...
	if httpResp.StatusCode >= 300 {
		// Enrich error with parsed fields if available (code/message/request_id)
		type errMeta struct {
		...
		msg := httpResp.Status
		if meta.Message != "" {
			msg = fmt.Sprintf("%s: %s", msg, meta.Message)
		}
		if meta.RequestID != "" {
			msg = fmt.Sprintf("%s (request_id=%s)", msg, meta.RequestID)
		}
